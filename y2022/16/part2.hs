...
shortestPaths :: Map String [String] -> String -> Map String [String]
shortestPaths graph source = shortestPath' Set.empty (Map.singleton source [])  [([], source)]
  where
    shortestPath' _       table []    = table
    shortestPath' visited table queue = case sortBy (compare `on` (fst >>> length)) queue of
      (p,n):queue'
        | Set.member n visited -> shortestPath' visited table queue'
        | otherwise            -> let
            neighbors = graph ! n
            table' = foldr (Map.alter (Just . maybe (p <> [n]) (minBy (compare `on` length) (p <> [d])))) table neighbors
...
search rooms = search' 30 "AA" "AA" []
  where
    graph = fmap _connections rooms 

    search' 0 _ _ = 0
    search' t cMe cEl v = if null cs then 0 else maximum [ val + search' (t - cost) next (next:v) | ((next, cost), val) <- cs,  ]
      where
        cs = c
          & shortestPaths graph
          & Map.toList
